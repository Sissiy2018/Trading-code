...
from signals import DefensiveSignalGenerator, ShortTermSignalGenerator, LongTermSignalGenerator, PCASignalGenerator, RobustRegressionBlender, RegimePCAHMMGenerator, VolumeConvictionGenerator, EPSForecastSignalGenerator
...
def run_regional_pipeline(region='US'):
    ...
    eps_gen = EPSForecastSignalGenerator(
        revision_window=cfg['eps']['revision_window'],
        revision_smooth_window=cfg['eps']['revision_smooth_window'],
        smoothing_span=cfg['eps']['smoothing_span'],
        dispersion_window=cfg['eps']['dispersion_window'],
        pct_smooth_window=cfg['eps']['pct_smooth_window'],
        clip_dispersion=cfg['eps']['clip_dispersion'],
        w_dispersion=cfg['eps']['w_dispersion'],
        w_revision=cfg['eps']['w_revision'],
        w_pct=cfg['eps']['w_pct'],
        pct_sign=cfg['eps']['pct_sign'],
        pct_cap=cfg['eps']['pct_cap'],
        pct_transform=cfg['eps']['pct_transform']
    )
    ...
    ic_volume = cfg['volume'].get('IC', cfg['volume'].get('ic', 0.0))
    ic_eps = cfg['eps'].get('IC', cfg['eps'].get('ic', 0.0))
    def get_sign(ic):
    ...
    signal_dict = {
        ...
        "eps": eps_gen.generate(
            closing_price=data.price_ret,
            eps_mean=data.eps_estimate_mean,
            eps_std=data.eps_estimate_std,
            eps_pct=data.eps_estimate_pct,
        ) * get_sign(ic_eps),
    }

    # 4. DYNAMIC PRIOR WEIGHTING BASED ON ABSOLUTE IC
    abs_ics = [abs(ic_short), abs(ic_long), abs(ic_pca), abs(ic_volume), abs(ic_regime), abs(ic_def), abs(ic_eps)]
    ...
    print(f"[{region}] Flipped? Short: {'Yes' if ic_short < 0 else 'No'}, Def: {'Yes' if ic_def < 0 else 'No'}, HMM: {'Yes' if ic_regime < 0 else 'No'}, EPS: {'Yes' if ic_eps < 0 else 'No'}")
    print(f"[{region}] Priors: Short:{priors[0]:.2f}, Long:{priors[1]:.2f}, PCA:{priors[2]:.2f}, Vol:{priors[3]:.2f}, Regime:{priors[4]:.2f}, Def:{priors[5]:.2f}, EPS:{priors[6]:.2f}")
    ...
    avg_horizon = (
        ...
        (cfg['eps'].get('horizon', 21) * priors[6])
    )
